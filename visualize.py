...
v3 在 v2 基础上改造价格趋势图，让"谁变了、变了多少"一眼可见：
  ...
  - 厂商简称：Kimi(Moonshot)→Kimi、豆包(Doubao)→豆包，避免 chips/图例过长
...
PROVIDER_ORDER = ["OpenAI", "Anthropic", "Google", "智谱 (Z.ai)",
                  "MiniMax", "Qwen", "Kimi (Moonshot)", "豆包 (Doubao)"]
...
const SHORT = {
  "智谱 (Z.ai)":"智谱", "Kimi (Moonshot)":"Kimi", "豆包 (Doubao)":"豆包"
};
...
function drawTrend(){
  ...
  const paid = DATA.filter(d=>!d.is_free && state.providers.has(d.provider));
  const ids = [...new Set(paid.map(d=>d.model_id))];
  const isIndex = state.mode==="index";

  const series = ids.map(id=>{
    const rows = DATA.filter(d=>d.model_id===id).sort((a,b)=>a.date<b.date?-1:1);
    const meta0 = rows[0];
    const isFlag = meta0.tier==="旗舰";
    const rawAt = dt=>{ const r=rows.find(x=>x.date===dt); return r? r[io] : null; };
    const base = rawAt(baseDate);
    const data = dates.map(dt=>{
      const v = rawAt(dt);
      if(v==null) return null;
      if(isIndex) return (base==null||base===0)? null : Number((v/base*100).toFixed(2));
      return v;
    });
    return {
      name: shortName(meta0.provider)+"·"+meta0.tier+" ("+meta0.model_short+")",
      type:"line", data, connectNulls:true, smooth:false, symbolSize:6,
      lineStyle:{ width: isFlag?2.6:1.8, type: isFlag?"solid":"dashed" },
      itemStyle:{ color: COLORS[meta0.provider]||"#999" },
    };
  });
  ...
  const valueFormatter = isIndex
    ? (v=>v==null?"—":v.toFixed(1)+"（"+(v>=100?"+":"")+(v-100).toFixed(1)+"%）")
    : (v=>v==null?"—":"$"+Number(v).toFixed(3)+"/M");
